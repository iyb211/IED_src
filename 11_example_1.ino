...
float dist_min, dist_max, dist_raw, dist_prev, dist_ema, alpha, prev_ema; // unit: mm
...
// get a distance reading from the USS
  ...
  dist_ema = alpha * dist_raw + (1-alpha) * prev_ema;
  prev_ema = dist_ema;
...
// adjust servo position according to the USS read value
  ...
  if(dist_ema < dist_min) {
    ...
    myservo.writeMicroseconds(_DUTY_MIN + (_DUTY_MAX-_DUTY_MIN)/(dist_max-dist_min)*(dist_ema-dist_min));
  }
